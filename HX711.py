...
class HX711:
    ...
    def __init__(self, dout, pd_sck, gain=128):
        ...
        self.SCK_lines = self.chip.get_line(self.PD_SCK)
        self.SCK_lines.request(consumer='HX711', type=gpiod.LINE_REQ_DIR_OUT)

        # Setup the GPIO Pin as input
        # GPIO.setup(self.DOUT, GPIO.IN)
        self.DOUT_lines = self.chip.get_line(self.DOUT)
        self.DOUT_lines.request(consumer='HX711', type=gpiod.LINE_REQ_DIR_IN)
    ...
    def read(self):
        ...
        while not (self.DOUT_lines.get_value() == 0):
        ...
        for i in range(24):
            ...
            self.SCK_lines.set_value(1)
            count = count << 1
            # GPIO.output(self.PD_SCK, False)
            self.SCK_lines.set_value(0)
            # if(GPIO.input(self.DOUT)):
            if(self.DOUT_lines.get_value()):
                count += 1
